<script lang="ts">
  ...
  const scaleData = (data: Point[], canvasHeight: number, canvasWidth: number, filledProportionX: number = 0.8, filledProportionY: number = 0.8): Point[] => {
    ...
    return data.map(p => ({x: p.x * xScale, y: p.y * yScale}));
  }
  ...
  const canvasReady = (args: any) => {
    ...
    const scaledData = scaleData(data, canvas.height, canvas.width);
